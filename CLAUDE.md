...
- `src/lib/walkthroughs/completion.ts` — completed guided-tour tracking (localStorage, per-browser) + `RECOMMENDED_WALKTHROUGH_ORDER` + `nextRecommendedWalkthrough()`; consumed by the walkthrough provider (completion toast chaining) and the Help Center Guided Tours card. Walkthrough/step `adminOnly` flags must match the launching article's `adminOnly`, and the recommended order must cover exactly the registered tours (both enforced by `integrity.test.ts`). `scripts/audit-walkthroughs.mts` steps every tour and asserts each step's highlight and button label — run it after changing tours or the pages they visit; ONE viewport per run (`npx tsx scripts/audit-walkthroughs.mts` for desktop, again with `VIEWPORT=mobile` for the phone layout), and it needs an authenticated admin browser context (auto-login sandbox)
