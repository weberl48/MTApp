...
`20260731_location_requirement_flags.sql` adds `service_types.requires_classroom` and `clients.requires_location` (applied to cert AND prod 2026-07-31, flags set the same day for the In-school service and the OLV/People Inc clients). These booleans drive the required session-location field (see `src/lib/session-location/resolve.ts` under Key Library Modules); the same-day picklist settings model (`custom_lists.locations_by_client` / `classrooms` / `classrooms_by_client`, `invoice.show_session_location`) is retired — stale JSONB keys in live org settings are ignored by the merge.
...
| `permissions` | `admin_view_contractor_pay`, `admin_view_margins`, `admin_view_analytics`, `admin_view_payroll` — what an ADMIN may see (owner-only; see Owner-only money surfaces) | All false |
...
- `team:view-rates` — Team page **Rates** tab (`PayRateMatrix`), the per-contractor **Rates** tab on `/team/[id]/`, the dashboard's Missing Contractor Rates card, **and every per-contractor earnings figure on the Team pages** (the list's Total Earned / Pending Pay columns and Pending Contractor Pay card; the detail page's Total Earnings / Paid Out / Pending Pay cards and its invoice **Contractor Pay** column).
...
**The first four are owner-CONFIGURABLE** (`2026-08-03-admin-financial-visibility-design.md`). `settings.permissions` holds four booleans, all default false, edited in Settings > Profile & Security ("What Admins Can See"); `adminGrantsFromSettings()` maps them to permissions and `canWithGrants(role, permission, grants)` ORs them onto `can()`. Rules that must hold: only the `admin` role is ever elevated (contractors never, owners already pass); only `ADMIN_GRANTABLE_PERMISSIONS` can be granted — `settings:edit` deliberately cannot, or an admin could grant themselves the rest; and `permissions` is absent from `ADMIN_WRITABLE_SETTING_SECTIONS` so `applySettingsUpdate()` discards an admin's write to it. `OrganizationContext.can()` is already grant-aware, so **client components must use the context's `can()`, never the raw import**. Code that can't reach the context — server components, API routes, and client pages whose authz runs inside their own fetch effect (a still-loading context would deny a granted admin) — calls `fetchAdminGrants(supabase, orgId)` from `src/lib/auth/admin-grants.ts`, which fails closed. This is UI gating only: RLS still lets an admin read `invoices.contractor_pay` over the wire.
...
- **Frontend** — `ErrorReporter` (`src/components/errors/error-reporter.tsx`, mounted unconditionally in the root layout) hooks `window.onerror`, `unhandledrejection` and `console.error`. It posts to `/api/dev/errors/` in dev and `/api/errors/` in production, and feeds `clientErrorBuffer` either way. `/api/errors/` requires an authenticated staff session on purpose: the client portal and login page are NOT covered, because an anonymous ingest endpoint on a public URL is a spam sink.
  **Two throttles, and both matter.** `createReportGate()` in `src/lib/errors/report.ts` caps each tab at 20 reports/min with a 5s dedupe — do not remove it, one render loop would otherwise write thousands of rows. Then `errorRateLimit` (40/60s, keyed by **user**) in the route. The proxy deliberately **excludes `/api/errors` from the shared per-IP `apiRateLimit`** (`isErrorSink` in `src/proxy.ts`): staff share one office NAT, so leaving telemetry in the 60/60s IP bucket let one person's error loop 429 everybody's real requests. Over-limit returns 204, not 429 — the caller is fire-and-forget.
...
**Bug reports** (`bug_reports` + the private `bug-screenshots` bucket, `20260807_bug_reports.sql`) are a separate table on purpose — attributed and voluntary, where `app_errors` is anonymous ambient telemetry. `submitBugReport()` (`src/app/actions/bug-reports.ts`) is the only write path: a browser can never insert directly and there is no INSERT policy, because two things need the server-only key. **Both `description` AND every `recent_errors[].message` are PHI-encrypted** — a message is whatever the app passed to `console.error`, so it gets the same treatment as the description; `kind` and `at` stay plaintext (machine-generated, and queryable without the key). Live column comments record this on the table itself.
...
- `src/lib/session-location/resolve.ts` — `resolveLocationField()`: the session-location rule — `service_types.requires_classroom` → required free-text **Classroom** (wins when both apply); any involved client's `clients.requires_location` (for group sessions: the Bill To agency) → required **Location**. The value lands in `sessions.classroom`, is snapshotted to `invoice_items.classroom` by the scholarship-batch paths, and prints on the invoice PDF + Square descriptions whenever present — there is deliberately no settings toggle (2026-07-31 rework; supersedes the same-day per-client picklist model)
