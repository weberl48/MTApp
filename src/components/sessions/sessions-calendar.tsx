...
export function SessionsCalendar({ sessions, isAdmin }: SessionsCalendarProps) {
  ...
  return (
    <div className="flex flex-col space-y-4">
                ...
                <div className="mt-2 space-y-1">
                  {daySessions.map((session) => (
                    <Link
                      ...
                      className="block"
                    >
                      <div className={`text-xs p-1.5 rounded border border-transparent hover:border-ring transition-colors ${sessionStatusColors[session.status] || 'bg-muted'}`}>
      ...
      <div className="md:hidden border rounded-lg overflow-hidden bg-card shadow-sm">
        {/* Days of week header */}
        <div className="grid grid-cols-7 border-b bg-muted/50">
          ...
          {calendarDays.map((day) => {
            ...
            return (
              <button
                ...
                className={`min-h-[52px] bg-card p-1 flex flex-col items-center transition-colors ${
                      ...
                      daySessions.map((s) => (
                        <span
                          ...
                          className={`w-1.5 h-1.5 rounded-full ${
                            s.status === 'submitted' ? 'bg-info'
                              : s.status === 'approved' ? 'bg-success'
                              : s.status === 'draft' ? 'bg-muted-foreground'
        ...
        <div className="md:hidden space-y-2">
          <h3 className="text-sm font-semibold text-foreground">
            ...
            {selectedDaySessions.length > 0 && (
              <span className="font-normal text-muted-foreground"> · {selectedDaySessions.length} session{selectedDaySessions.length !== 1 ? 's' : ''}</span>
            )}
          </h3>
          {selectedDaySessions.length > 0 ? (
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <Badge className={`shrink-0 text-xs px-1.5 py-0 ${sessionStatusColors[session.status]}`}>
